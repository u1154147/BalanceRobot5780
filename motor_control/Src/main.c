...
int main(int argc, char* argv[]) {
    ...
    button_init();                          // Initialize button
		...
		NVIC_SetPriority(5, 3); // Set priority of EXTI0 to 3. Set TIMER6 interrupt to higher than this

		//NVIC_SetPriority(17, 2); // Set priority of TIIMER6 to 1 
    ...
    while (1) {
        ...
        encoder_count = TIM2->CNT;
				...
				if (x > threshold && ccw == 0)
					target_rpm += 5;
				
				else if (x > threshold && ccw == 1)
				{
					target_rpm = 5;
					change_direction();
				}
				
				else if (x < -threshold && ccw == 0)
				{
					target_rpm = 5;
					change_direction();
				}
        ...
        HAL_Delay(64);                      // Delay 1/16 second
    }
}
